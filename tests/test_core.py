...
def test_sequence_nested() -> None:
    common(
        Sequence(Int8ub, Int16ub, Sequence(Int8ub, Int8ub)),
        b"\x01\x00\x02\x03\x04",
        [1, 2, [3, 4]],
        5,
    )
...
def test_prefixed() -> None:
    ...
    common(d5, b"\x0a" + bytes(10), "\x00" * 10, SizeofError)
...
def test_restreamdata_issue_701() -> None:
    ...
    assert d1.parse(b"\x01GGGGGGGGGG\x02") == Container(
...
def test_transformed() -> None:
    ...
    common(d3, bytes(2), "\x00" * 16, SizeofError)
...
def test_processxor() -> None:
    ...
    common(d7, b"\x00", "X", SizeofError)
    d8 = ProcessXor(b"XXXXX", GreedyString("utf-8"))
    common(d8, b"\x00", "X", SizeofError)
            ...
            def _parse(self, stream, context, path):
                ...
                if hash1 != hash2:
                    ...
                    warnings.warn(
                        ...
                        % (
                            hash1
                            if not isinstance(hash1, bytestringtype)
                            else binascii.hexlify(hash1),
                            hash2
                            if not isinstance(hash2, bytestringtype)
                            else binascii.hexlify(hash2),
                            path,
                        ),
                        ChecksumWarning,
                    )
...
@pytest.mark.xfail(
    ONWINDOWS and PYPY,
    reason="no wheel for 'cryptography' is currently available for pypy on windows",
)
...
def test_encryptedsymaead() -> None:
    ...
    assert (
        raises(EncryptedSymAead(GreedyBytes, "AESGCM", bytes(16)).build, b"")  # type: ignore
        == CipherError
    )
    assert (
        raises(EncryptedSymAead(GreedyBytes, "AESGCM", bytes(16)).parse, b"")  # type: ignore
        == CipherError
    )
...
def test_from_issue_246() -> None:
    ...
    assert NumVertices2.parse(b"\x01\x34\x56\x70") == Container(
...
def test_from_issue_244() -> None:
    if t.TYPE_CHECKING:
        ...
        class AddIndexes(
            Adapter[
                ListContainer[t.Any], t.List[t.Any], ListContainer[t.Any], t.List[t.Any]
            ]
        ): ...
    else:
...
def test_from_issue_324() -> None:
    d = Struct(
        ...
        "checksum" / Checksum(Byte, lambda data: int(sum(data)) & 0xFF, this.vals.data),
    )
...
def test_exposing_members_context() -> None:
    ...
    assert d4.parse(b"\x01\x02\x03\x04") == dict(
...
def test_parsedhook_repeatersdiscard() -> None:
    ...
    d3 = RepeatUntil(
        lambda obj, lst, ctx: ctx._index == 2, Byte * printobj3, discard=True
    )
...
def test_exportksy() -> None:
    d = (
        Struct(
            ...
            "namedtuple1"
            / NamedTuple("coord", "x y z", "x" / Byte + "y" / Byte + "z" / Byte),
...
def test_parsetype_covariance() -> None:
    ...
    if t.TYPE_CHECKING:
        ...
        class ReversedList(
            SymmetricAdapter[t.List[T], t.List[T], t.List[T], t.List[T]]
        ): ...
    else:
